...
function load_list(data) {
    if (data && data.data) {
        ...
        for (var i = 0, len = arr.length; i < len; i++) {
            ...
            ;
            if (file.isDir) {
                item = item.format("index.html?oauth=" + getOauth() + "&path=" + file.path, "folder", file.name, new Date(file.mtime).format("yyyy-MM-dd hh:mm:ss"), "", file.path, !file.isDir, file.name , "i", "");
            } else {
                var fix = "i";
                var file_src = "";
                ...
                item = item.format("/src" + file.path + "?oauth=" + getOauth(), getFileTypeCss(file.name), file.name, new Date(file.mtime).format("yyyy-MM-dd hh:mm:ss"), renderSize(file.size), file.path, !file.isDir, file.name, fix, file_src);
            }
            $('#content').append(item);
